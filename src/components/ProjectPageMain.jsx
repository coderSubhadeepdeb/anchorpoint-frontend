...
const ProjectPageMain = () => {
  ...
  useEffect(() => {
    async function fetchProject() {
      try {
        const response = api.get('/api/v1/projects'); // Replace with your actual endpoint
        const result = await response.json();
        
        if (result.success && result.data) {
          const project = result.data;
          
          // Transform the project into your desired images array format
          const transformedImages = project.images.map(imageUrl => ({
            src: imageUrl[0],
            aspect: "4/3", // You can set a default or calculate dynamically
            section: project.category, // Using project's category
            name: project.title // Using project's title
          }));
